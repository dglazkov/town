...
it("walks journey 1: a town exported with a key made, imported into a directory that did not exist, served, and its grant file working at the new address", async () => {
  ...
  const auditB = lines(ok(second.admin("audit")).stdout);
  expect(auditB.slice(0, 1 + calls)).toEqual(lines(auditBefore));
  ...
  for (const verb of [["user", "ls"], ["pass", "ls"], ["grant", "ls"], ["type", "ls"], ["credential", "ls", "--user", "dimitri"], ["permit", "ls"]]) {
    ...
    if (verb[0] === "pass" || verb[0] === "grant") expect(lines(y).map((l) => l.split(/\s{2,}/).slice(0, -1)), verb.join(" ")).toEqual(lines(x).map((l) => l.split(/\s{2,}/).slice(0, -1)));
    else expect(y, verb.join(" ")).toBe(x);
  }
